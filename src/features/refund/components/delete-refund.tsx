...
const DeleteRefund = ({ onClose, open, refund }: deleteSocialProps) => {
  const token = getToken();
  const {  deleteRefund, fail, pending } = useDeleteRefund(token ?? "");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showToast("Suppression reussie !", "success");
      onClose();
    } catch (e) {
    ...
    <Modal>
      ...
      <form onSubmit={handleSubmit}>
        <p className="text-gray-500 text-sm">
          Voulez-vous vraiment supprimer ce remboursement ?{" "}
