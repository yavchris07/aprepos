...
const DeleteLoan = ({ onClose, open, loan }: deleteLoanProps) => {
  const token = getToken();
  const { deleteLoan, fail, pending } = useDeleteLoan(token ?? "");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showToast("Suppression emprunt reussie !", "success");
      onClose();
    } catch (e) {
  ...
  return (
    <Modal>
      <div className="flex justify-between items-center my-2">
        <h2 className="text-black font-semibold">Suppression emprunt</h2>
